...
export default function Banner(){
    ...
    return (
    <div className="overflow-hidden h-12 bg-[#cafc01] flex items-center mt-6 ">
      <div className="banner whitespace-nowrap  py-2 animate-scrollBanner ease-linear infinite">
        {config.map((message, index) => (
          <span key={index} className="text-black text-lg duration-[40s] font-medium p-3">{message} - <FontAwesomeIcon icon={faFaceSmile} /> </span>
        ))}
      </div>
    </div>
    );
